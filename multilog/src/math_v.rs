...
verus! {
    ...
    */
    ...
    pub proof fn lemma_mod_between_alt(d: int, x: int, y: int, z: int)
        requires
            d > 0,
            x % d <= y % d,
            y - x < d,
    ...
    pub proof fn lemma_mod_not_between_alt(d: int, x: int, y: int, z: int)
        requires
            d > 0,
            x < y <= z,
            z - x < d,
            ...
            x % d < y % d || x % d > z % d
    {
        if y % d <= x % d <= z % d {
            ...
            lemma_div_relation_when_mods_have_different_order(d, x, y);
            assert(y / d == x / d + 1);
            lemma_div_relation_when_mods_have_same_order_alt(d, x, z);
            assert(z / d == x / d);
            assert(x < y <= z);
            assert(d * (y / d) + y % d <= d * (z / d) + z % d);
            assert(d * (x / d + 1) + y % d <= d * (x / d) + z % d);
            lemma_mul_is_distributive_add(d, x / d, 1);
            assert(d * (x / d) + d * 1 + y % d <= d * (x / d) + z % d);
            assert(d * 1 + y % d <= z % d);
            assert(false);
        }
    }
    ...
    pub proof fn lemma_values_that_differ_by_multiple_have_same_mod(d: int, x: int, y: int, z: int)
        requires
            d > 0,
            x - y == d * z,
        ensures
            x % d == y % d
    {
        assert (x == d * z + y);
        lemma_fundamental_div_mod(y, d);
        assert (x == d * z + d * (y / d) + y % d);
        lemma_mul_is_distributive_add(d, z, y / d);
        assert (x == d * (z + y / d) + y % d);
        lemma_fundamental_div_mod_converse(x, d, z + y / d, y % d);
    }

    pub proof fn lemma_mod_of_multiple_is_zero(d: int, x: int)
        requires
            d > 0
        ensures
            (d * x) % d == 0
    {
        lemma_values_that_differ_by_multiple_have_same_mod(d, d * x, 0, x);
    }

    pub proof fn lemma_value_congruent_to_mod(d: int, x: int)
        requires
            d > 0
        ensures
            x % d == (x % d) % d
    {
        lemma_fundamental_div_mod(x, d);
        lemma_values_that_differ_by_multiple_have_same_mod(d, x, x % d, x / d);
    }

    pub proof fn lemma_sum_congruent_to_sum_of_mods(d: int, x: int, y: int)
        requires
            d > 0
        ensures
            (x + y) % d == (x % d + y % d) % d
    {
        ...
        lemma_values_that_differ_by_multiple_have_same_mod(d, x + y, x % d + y % d, x / d + y / d);
    }

    pub proof fn lemma_sum_congruent_to_sum_of_congruents(d: int, x1: int, y1: int, x2: int, y2: int)
        requires
            d > 0,
            x1 % d == x2 % d,
            y1 % d == y2 % d,
        ensures
            (x1 + y1) % d == (x2 + y2) % d
    {
        lemma_sum_congruent_to_sum_of_mods(d, x1, y1);
        lemma_sum_congruent_to_sum_of_mods(d, x2, y2);
    }

    pub proof fn lemma_sum_congruent_to_sum_with_mod(d: int, x: int, y: int)
        requires
            d > 0
        ensures
            (x + y) % d == (x + y % d) % d
    {
        lemma_value_congruent_to_mod(d, y);
        lemma_sum_congruent_to_sum_of_congruents(d, x, y, x, y % d);
    }

    pub proof fn lemma_minus_mod_congruent_to_zero(d: int, x: int)
        requires
            d > 0
        ensures
            (x - x % d) % d == 0
    {
        lemma_fundamental_div_mod(x, d);
        lemma_mod_of_zero_is_zero(d);
        lemma_values_that_differ_by_multiple_have_same_mod(d, x - x % d, 0, x / d);
    }

    pub proof fn lemma_diff_congruent_to_diff_of_mods(d: int, x: int, y: int)
        requires
            d > 0
        ensures
            (x - y) % d == (x % d - y % d) % d
    ...
    pub proof fn lemma_diff_congruent_to_diff_of_congruents(d: int, x1: int, y1: int, x2: int, y2: int)
        requires
            d > 0,
            x1 % d == x2 % d,
            y1 % d == y2 % d,
        ensures
            (x1 -y1) % d == (x2 - y2) % d
    {
        lemma_diff_congruent_to_diff_of_mods(d, x1, y1);
        lemma_diff_congruent_to_diff_of_mods(d, x2, y2);
    }

    pub proof fn lemma_diff_congruent_to_diff_with_mod(d: int, x: int, y: int)
        requires
            d > 0
        ensures
            (x - y) % d == (x - y % d) % d
    {
        lemma_value_congruent_to_mod(d, y);
        lemma_diff_congruent_to_diff_of_congruents(d, x, y, x, y % d);
    }
}

...
verus! {
    ...
    pub fn read_cdb<PMRegions: PersistentMemoryRegions>(pm_regions: &PMRegions) -> (result: Result<bool, MultiLogErr>)
        requires
            pm_regions.inv(),
            pm_regions@.len() > 0,
            recover_cdb(pm_regions@[0].committed()) is Some,
            ...
            ({
                ...
                match result {
                    ...
                    Err(MultiLogErr::CRCMismatch) =>
                        state is Some ==> !pm_regions.constants().impervious_to_corruption,
                    _ => state is None
                }
            })
    {
        ...
        if region_size < ABSOLUTE_POS_OF_LOG_AREA + MIN_LOG_AREA_SIZE {
            assert(state is None); // This can't happen if the persistent memory is recoverable
            ...
            }
    {
                ...
                cdb == deserialize_and_check_log_cdb(pm_regions@[0].committed()).unwrap(),
        {
            ...
            assert (region_state is Some) by {

...
verus! {
    ...
    impl <PMRegions: PersistentMemoryRegions> MultiLogImpl<PMRegions> {
        ...
        pub exec fn start(pm_regions: PMRegions, multilog_id: u128)
            ...
            requires
                pm_regions.inv(),
                UntrustedMultiLogImpl::recover(pm_regions@.flush().committed(), multilog_id) is Some,
            ensures
                match result {
                    Ok(trusted_log_impl) => {
                ...
                }
        {
            ...
            let ghost state = UntrustedMultiLogImpl::recover(pm_regions@.flush().committed(), multilog_id).unwrap();

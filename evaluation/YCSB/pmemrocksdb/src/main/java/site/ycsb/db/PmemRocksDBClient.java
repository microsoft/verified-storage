...
/**
 * Bindings for pmem-rocksdb. This class is (almost) identical
 * to the regular RocksDB bindings, but it uses pmem-rocksdb
 * as the database.
 *
 * See {@code rocksdb/README.md} for details.
 */
public class PmemRocksDBClient extends DB {
  ...
  private static final Logger LOGGER = LoggerFactory.getLogger(PmemRocksDBClient.class);

  @GuardedBy("PmemRocksDBClient.class") private static Path rocksDbDir = null;
  @GuardedBy("PmemRocksDBClient.class") private static Path optionsFile = null;
  @GuardedBy("PmemRocksDBClient.class") private static RocksObject dbOptions = null;
   ...
   *
   * Should only be called with a {@code synchronized(PmemRocksDBClient.class)` block}.
   *
  ...
  private RocksDB initRocksDB() throws IOException, RocksDBException {
    ...
    // final int rocksThreads = Runtime.getRuntime().availableProcessors();

    // System.out.println("ROCKS THREADS: " + rocksThreads);

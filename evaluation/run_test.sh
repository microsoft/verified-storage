...
mkdir -p $RESULTS_DIR/$DB/trace_workload1
check_error $?
...
for iter in $(seq $iterations); do 
    ...
    ./bin/ycsb load $DB -threads 1 -s -P ../workloads/trace_workload1 -p recordcount=$RECORD_COUNT -p operationcount=$OP_COUNT $options > ../$RESULTS_DIR/$DB/trace_workload1/Load$iter
    check_error $?

...
impl<const N: usize, const M: usize> DurableBlockList<N, M> {
    ...
    //             let addr =
    //                 BlockListTable::<N, M>::row_offset_in_block(self.tail_addr, new_row_index);
    ...
    //             let addr = BlockListTable::row_offset_in_block(new_block_addr, 0);
    //             let new_element = DurableBlockListRow::new(*val);
    //             let new_element_bytes = new_element.to_bytes();
    //             mem_pool.write(addr, new_element_bytes)?;
    ...
    //             mem_pool.write(
    //                 BlockListTable::get_next_pointer_offset(new_block_addr),
    //                 &next_bytes,
    //             )?;
    //             mem_pool.flush();
    ...
    //             let next_new_offset = BlockListTable::get_next_pointer_offset(old_tail_block_addr);
    ...
    //     let next_ptr_addr = Self::get_next_pointer_offset(addr);

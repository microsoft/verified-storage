...
verus! {
/// A `WriteRestrictedPersistentMemoryRegion` is a wrapper around a
/// persistent memory region that restricts how it can be written.
/// Specifically, it only permits a write if it's accompanied by a
/// tracked permission authorizing that write. The tracked permission
/// must authorize every possible state that could result from
/// crashing while the write is ongoing.

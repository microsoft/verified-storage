    ...
    L: PmCopy + LogicalRange + std::fmt::Debug + Copy,
{
    ...
    {
        let overhead = CheckedU64::new(Journal::<Perm, PermFactory, PM>::journal_entry_overhead());
            ...
            },
    {
        ...
        let journal_end = Journal::<Perm, PermFactory, PM>::space_needed_for_setup(&journal_capacity);
        ...
        let key_table_size = KeyTable::<Perm, PermFactory, PM, K>::space_needed_for_setup(ps, &sm_crc_end);
        let key_table_end = sm_crc_end.add_checked(&key_table_size);
        let item_table_size = ItemTable::<Perm, PermFactory, PM, I>::space_needed_for_setup(ps, &key_table_end);
        let item_table_end = key_table_end.add_checked(&item_table_size);
        let list_table_size = ListTable::<Perm, PermFactory, PM, L>::space_needed_for_setup(ps, &item_table_end);
            ...
            },
    {
        ...
        let key_sm = match KeyTable::<Perm, PermFactory, PM, K>::setup(pm, ps, sm_crc_end.unwrap(), pm_size) {
            Ok(key_sm) => key_sm,
            Err(e) => { return Err(e); },
        };
        let ghost state_after_key_init = pm@.read_state;
    
        let item_sm = match ItemTable::<Perm, PermFactory, PM, I>::setup(pm, ps, key_sm.end(), pm_size) {
            Ok(item_sm) => item_sm,
            Err(e) => { return Err(e); },
        };
        let ghost state_after_item_init = pm@.read_state;
    
        let list_sm = match ListTable::<Perm, PermFactory, PM, L>::setup(pm, ps, item_sm.end(), pm_size) {
            Ok(list_sm) => list_sm,
            Err(e) => { return Err(e); },
        };
        ...
        match Journal::<Perm, PermFactory, PM>::setup(pm, &jc) {
            Ok(jc) => {},

...
verus! {
    ...
    pub proof fn lemma_main_table_recovery_after_updating_entry<K>(
        ...
        requires
            ...
            forall|i: int| 0 <= i < num_keys ==> {
                (#[trigger] parse_main_table::<K>(mem1, num_keys, main_table_entry_size).unwrap().durable_main_table[i])
                matches Some(e) ==> e.entry.item_index != entry.item_index
            },
            mem1.len() == mem2.len(),
            index < num_keys,
            forall|addr: int| {
            ...
            }),
    {
        ...
        let table1 = parse_main_table::<K>(mem1, num_keys, main_table_entry_size).unwrap();

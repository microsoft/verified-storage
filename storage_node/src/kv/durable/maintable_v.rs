            ...
            K: PmCopy,
    {
        ...
        pub open spec fn delete(self, index: int) -> Option<Self>
        {
            if index < 0 || index >= self.durable_main_table.len() {
                ...
                // match self.durable_main_table[index] {
                //     Some(_) => {
                //         Some(Self {
                //             durable_main_table: self.durable_main_table.update(index, None)
                //         })
                ...
                //     _ => None
                // }
                // if self.durable_main_table@[i] is None {
                //     None
                // } else {
                //     Some(Self {
                //         durable_main_table: self.durable_main_table.set(i, None)
                //     })
                // }
            }
        }

        // pub closed spec fn spec_index(self, index: int) -> Option<MainTableViewEntry<K>> {
        //     if 0 <= index < self.main_table.len() {
        //         self.main_table[index]
        //     } else {
        //         None
        //     }
        // }
            ...
            K: PmCopy + std::fmt::Debug,
    {
                ...
                }
        {
            ...
            assert forall |k: nat| k < num_keys implies {
                validate_metadata_entry::<K>(#[trigger] extract_bytes(mem, (k * main_table_entry_size) as nat,
            ...
            proof {
                let entries = parse_metadata_entries::<K>(mem, num_keys as nat, main_table_entry_size as nat);
...
metadata_allocator@.contains(i)
                ...
                }
        {
            ...
            //     self@.durable_main_table[idx as int] is None 
            // by { assert(old(self).main_table_free_list@.contains(idx)); }

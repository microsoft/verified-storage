...
verus! {
            ...
            L: PmCopy + std::fmt::Debug + Copy,
    {
        ...
        // TODO @hayley better differentiate this from tentative view / figure out 
        // exactly how you want to handle these
        pub closed spec fn outstanding_view(self) -> DurableKvStoreView<K, I, L>
        {
            // TODO @hayley should probably also be using a tentative view of the item table
            Self::recover_from_component_views(self.main_table.tentative_view(), self.item_table@)
        }
        ...
        pub closed spec fn key_index_list_view(self) -> Set<(K, u64, u64)>
        {
            Set::new(
                |val: (K, u64, u64)| {
                    exists |j: u64| {
                        &&& 0 <= j < self.main_table@.contents.len()
                        &&& #[trigger] self.main_table@.contents[j as int] matches Some(entry)
        ...
        {
            Self::tentative_main_table_given_log_valid(self.wrpm@.flush().committed(), self.log@.physical_op_list, self.overall_metadata)
        }

        pub closed spec fn tentative_main_table_given_log_valid(bytes: Seq<u8>, log: Seq<AbstractPhysicalOpLogEntry>, overall_metadata: OverallMetadata) -> bool 
        {
            let tentative_state_bytes =
                apply_physical_log_entries(bytes, log);
            let tentative_main_table_bytes =
                extract_bytes(tentative_state_bytes.unwrap(), overall_metadata.main_table_addr as nat,
                            overall_metadata.main_table_size as nat);
            let tentative_main_table =
                parse_main_table::<K>(tentative_main_table_bytes, overall_metadata.num_keys,
        ...
        {
            Self::tentative_main_table_given_log(self.wrpm@.flush().committed(), self.log@.physical_op_list, self.overall_metadata)
        }

        pub closed spec fn tentative_main_table_given_log(bytes: Seq<u8>, log: Seq<AbstractPhysicalOpLogEntry>, overall_metadata: OverallMetadata) -> MainTableView<K>
        {
            let tentative_state_bytes = apply_physical_log_entries(bytes, log);
            let tentative_main_table_bytes =
                extract_bytes(tentative_state_bytes.unwrap(), overall_metadata.main_table_addr as nat,
                            overall_metadata.main_table_size as nat);
            let tentative_main_table =
        ...
        pub closed spec fn tentative_item_table_valid(self) -> bool
        {
            ...
            let tentative_item_table =
                parse_item_table::<I, K>(tentative_item_table_bytes,
                                         ...
                                         tentative_main_table.unwrap().valid_durable_item_indices());
            &&& tentative_state_bytes is Some
            &&& tentative_main_table is Some
        ...
        pub closed spec fn abort_inv(self) -> bool 
        {
            ...
            let valid_durable_item_indices = self.main_table@.valid_durable_item_indices();

            &&& forall |idx: u64| 0 <= idx < self.main_table@.contents.len() ==> 
            ...
            &&& forall |idx: u64| 0 <= idx < self.overall_metadata.num_keys ==> 
                    item_table_allocator_state.pending_alloc_check(idx, valid_durable_item_indices, valid_durable_item_indices)
        ...
        pub closed spec fn inv(self) -> bool 
        {
            ...
            &&& self.item_table.inv(get_subregion_view(pm_view, self.overall_metadata.item_table_addr as nat,
                                                     self.overall_metadata.item_table_size as nat),
                                  self.overall_metadata, self.main_table@.valid_durable_item_indices())
        ...
        pub closed spec fn recovery_inv(self) -> bool
        {
            // replaying the current log results in the main table's current
            // tentative view
            let mem = self.wrpm@.flush().committed();
            let op_log = self.log@.commit_op_log().physical_op_list;
            let mem_with_log_installed = apply_physical_log_entries(mem, op_log);
            if let Some(mem_with_log_installed) = mem_with_log_installed {
                let main_table_region = extract_bytes(mem_with_log_installed, 
                ...
                if let Some(main_table_view) = main_table_view {
                    main_table_view == self.main_table.tentative_view()
                } else {
                    false
                }
            } else {
        ...
        pub closed spec fn valid(self) -> bool 
        {
            ...
            &&& self.item_table.valid(get_subregion_view(pm_view, self.overall_metadata.item_table_addr as nat,
                                                       self.overall_metadata.item_table_size as nat),
                                    self.overall_metadata,
                                    self.main_table@.valid_durable_item_indices())
        ...
        pub closed spec fn pending_alloc_inv(self) -> bool
        {
            ...
            if let Some(tentative_state_bytes) = tentative_state_bytes {
                ...
                &&& self.main_table@.valid_durable_item_indices() == durable_main_table_view.valid_durable_item_indices()
                &&& self.item_table.pending_alloc_inv(
                        durable_main_table_view.valid_durable_item_indices(),
                        tentative_main_table_view.valid_durable_item_indices(),
                    )
            } else {
                false
            } 
        ...
        pub closed spec fn pending_alloc_inv2(self) -> bool 
        {
            self.main_table.pending_alloc_inv2()
        }
            ...
            out == self.spec_version_metadata()
    {
        ...
        pub open spec fn physical_recover_after_applying_log(mem_with_log_installed: Seq<u8>, overall_metadata: OverallMetadata) -> Option<DurableKvStoreView<K, I, L>>
        {
            ...
            if let Some(main_table_view) = main_table_view {
                let item_table_view = parse_item_table::<I, K>(
                    ...
                    main_table_view.valid_durable_item_indices()
                );
        ...
        proof fn lemma_transaction_abort(self, old_self: Self)
            requires
                ...
                old_self.main_table@.valid_durable_item_indices() == self.main_table@.valid_durable_item_indices(),
            ensures
                ({
        ...
        proof fn lemma_if_states_differ_only_in_log_region_after_log_replay_then_views_match(
            s1: Seq<u8>,
            s2: Seq<u8>,
            overall_metadata: OverallMetadata,
        )
            requires 
                ...
                states_differ_only_in_log_region(s1, s2, overall_metadata.log_area_addr as nat,
                    overall_metadata.log_area_size as nat),
            ensures 
                Self::physical_recover_after_applying_log(s1, overall_metadata) == 
                    Self::physical_recover_after_applying_log(s2, overall_metadata),
                extract_bytes(s1, overall_metadata.main_table_addr as nat, 
                        overall_metadata.main_table_size as nat) == 
                    extract_bytes(s2, overall_metadata.main_table_addr as nat, 
                        overall_metadata.main_table_size as nat)
        {
        ...
        ) -> DurableKvStoreView<K, I, L>
        {
            let contents = Map::new(
                |i: int| 0 <= i < recovered_main_table.contents.len() && recovered_main_table.contents[i] is Some,
                |i: int| {
                    let main_table_entry = recovered_main_table.contents[i].unwrap();
        ...
        proof fn lemma_if_key_missing_from_tentative_view_then_missing_from_tentative_main_table(self, key: K)
            ...
            ensures
                self.tentative_main_table_valid(),
                ({
                    let t = self.tentative_main_table().contents;
                ...
                })
        {
            ...
            assert(no_duplicate_keys(self.main_table@.contents));
            ...
            assert forall |k| #[trigger] key_to_index.contains_key(k) implies {
                ...
                let i = key_to_index[k];
                assert(self.main_table@.contents[i] is Some ==> self@.contains_key(i));
            }
        }
                ...
                }
        {
            ...
            proof {
                ...
                let valid_indices = recovered_main_table.unwrap().valid_durable_item_indices();
                ...
                assert(forall |i: int| 0 <= i < recovered_main_table.unwrap().contents.len() ==> #[trigger] recovered_main_table.unwrap().contents[i] is None);

                DurableList::<K, L>::lemma_parse_each_list_succeeds_if_no_valid_metadata_entries(
                    recovered_main_table.unwrap().contents,
                    list_area_bytes,
                    Map::empty(),
                    overall_metadata.list_node_size,
                    overall_metadata.num_list_entries_per_node,
                ...
                }
        {
            ...
            assert(durable_kv_store.pending_alloc_inv()) by {
                ...
                let tentative_state_bytes = apply_physical_log_entries(wrpm_region@.flush().committed(),
            ...
            proof {
                ...
                assert(parse_item_table::<I, K>(item_table_subregion.view(pm_region).committed(), overall_metadata.num_keys as nat, main_table@.valid_durable_item_indices()).unwrap() == item_table@);
                   /* REMOVED UNTIL WE IMPLEMENT LISTS
                ...
                assert forall |i: u64| #[trigger] durable_kv_store@.contains_key(i as int) implies {
                    ...
                    let entry = durable_kv_store.main_table@.contents[i as int].unwrap();
                ...
                assert(durable_kv_store.main_table.tentative_view() == durable_kv_store.main_table@);

                let main_table_region = extract_bytes(durable_kv_store.wrpm@.flush().committed(), 
                    durable_kv_store.overall_metadata.main_table_addr as nat, durable_kv_store.overall_metadata.main_table_size as nat);
                assert(main_table_subregion.view(pm_region).committed() == main_table_region);
            }
                ...
                }
        {
            ...
            self.item_table.read_item(
                ...
                item_table_index,
                Ghost(self.overall_metadata),
                Ghost(self.main_table@.valid_durable_item_indices()),
            )
        }
        ...
        closed spec fn condition_preserved_by_subregion_masks(self) -> (spec_fn(Seq<u8>) -> bool)
        {
            ...
            |s: Seq<u8>| {
                ...
                &&& parse_item_table::<I, K>(extract_bytes(s, item_table_addr as nat, item_table_size as nat),
                                            num_keys as nat, self.main_table@.valid_durable_item_indices()) ==
                ...
                )
        {
            ...
            assert forall|s1: Seq<u8>, s2: Seq<u8>| {
                ...
                assert forall|addr: int| 0 <= addr < ss1.len() && ss1[addr] != #[trigger] ss2[addr] implies
                    ...
                    assert(self.main_table@.contents[which_entry as int] is None);
        ...
        spec fn get_writable_mask_for_item_table(self) -> (mask: spec_fn(int) -> bool)
        {
            |addr: int| address_belongs_to_invalid_item_table_entry::<I>(
                ...
                self.main_table@.valid_durable_item_indices().union(self.tentative_main_table().valid_durable_item_indices())
            )
        }
            ...
            assert forall|s1: Seq<u8>, s2: Seq<u8>| {
                ...
                assert forall|addr: int| 0 <= addr < ss2.len() && ss1[addr] != #[trigger] ss2[addr] implies
                           address_belongs_to_invalid_item_table_entry::<I>(
                               addr, num_keys, self.main_table@.valid_durable_item_indices()
                           ) by {
                }
                ...
                self.tentative_item_table() == old_self.tentative_item_table(),
        {
            ...
            let old_tentative_item_table_parsed =
                parse_item_table::<I, K>(old_tentative_item_table_bytes, num_keys as nat,
                                         old_tentative_main_table_parsed.valid_durable_item_indices());
            ...
            assert(old_self.item_table.pending_alloc_inv(old_durable_main_table_parsed.valid_durable_item_indices(),
                                                         old_tentative_main_table_parsed.valid_durable_item_indices()));
            assert(old_self.item_table.allocator_view().pending_alloc_check(
                item_index,
                old_durable_main_table_parsed.valid_durable_item_indices(),
                old_tentative_main_table_parsed.valid_durable_item_indices()
            ));
            assert(old_self.item_table.free_list().contains(item_index));
            assert(!old_tentative_main_table_parsed.valid_durable_item_indices().contains(item_index));
            assert(!old_self.main_table@.valid_durable_item_indices().contains(item_index));
            ...
            assert forall|idx: u64| idx < old_tentative_main_table_parsed.len() implies
                   (#[trigger] old_tentative_main_table_parsed.contents[idx as int] matches Some(e) ==>
            ...
            assert(!new_tentative_main_table_parsed.valid_durable_item_indices().contains(item_index));
            ...
            assert forall|addr: int| {
                ...
                addr, num_keys, new_tentative_main_table_parsed.valid_durable_item_indices()
            ) by {
            ...
            lemma_parse_item_table_doesnt_depend_on_fields_of_invalid_entries::<I, K>(
                ...
                num_keys,
                new_tentative_main_table_parsed.valid_durable_item_indices(),
            );

            let new_tentative_item_table_parsed =
                parse_item_table::<I, K>(new_tentative_item_table_bytes, num_keys as nat,
                                         new_tentative_main_table_parsed.valid_durable_item_indices()).unwrap();
        ...
        proof fn lemma_metadata_pending_allocs_are_invalid_at_abort(self)
            ...
            ensures 
                forall |idx: u64| #[trigger] self.main_table.pending_allocations_view().contains(idx) ==> {
                    self.main_table@.contents[idx as int] is None
                }
        {
            let durable_main_table_subregion_view = get_subregion_view(self.wrpm@, 
                self.overall_metadata.main_table_addr as nat, self.overall_metadata.main_table_size as nat);
            ...
            assert forall |idx: u64| #[trigger] self.main_table.pending_allocations_view().contains(idx) implies {
                durable_main_table_view.contents[idx as int] is None
            } by {
                ...
                self.tentative_view() == Some(self@),
        {
            ...
            proof {
                ...
                old_self.lemma_metadata_pending_allocs_are_invalid_at_abort();
            }
        ...
        proof fn lemma_helper_for_justify_validify_log_entry(
            ...
            requires
                ...
                self.main_table.allocator_inv(),
                self.main_table@.contents == self_before_main_table_create.main_table@.contents,
                ...
                ({
                    &&& self.main_table.outstanding_entry_writes[main_table_index] matches Some(e)
                    &&& e.entry matches EntryStatus::Created(entry)
                    &&& e.key == key
                    &&& entry.head == list_node_index
                    &&& entry.tail == list_node_index
                    &&& entry.length == 0
                    &&& entry.first_entry_offset == 0
                    &&& entry.item_index == item_index
                }),
                ({
                    let t = self_before_main_table_create.tentative_main_table().contents;
                    forall|i: int| 0 <= i < t.len() && #[trigger] t[i] is Some ==> t[i].unwrap().key != key
                }),
                !old_self.tentative_main_table().valid_durable_item_indices().contains(item_index),
            ensures
                self.log.inv(self.wrpm@, self.version_metadata, self.overall_metadata),
                ...
                ({
                    ...
                    // let entry = self.main_table.outstanding_entry_writes[main_table_index].unwrap();
                    ...
                    let e = self.main_table.outstanding_entry_writes[main_table_index].unwrap();
                    let metadata_bytes2 = ListEntryMetadata::spec_to_bytes(e.entry->Created_0);
                    ...
                    &&& metadata_bytes == metadata_bytes2
                ...
                ({
                    ...
                    let entry = self.main_table.outstanding_entry_writes[main_table_index].unwrap();
                    let item_index = entry.entry->Created_0.item_index;
                    &&& mem.len() == overall_metadata.region_size
                    ...
                    &&& self.main_table@.contents[main_table_index as int] is None
                    &&& self.main_table.outstanding_entry_writes[main_table_index] is Some
                    ...
                    &&& metadata_bytes == entry.entry->Created_0.spec_to_bytes()
                    ...
                    &&& key == entry.key
                    &&& metadata == entry.entry->Created_0
                ...
                ({
                    ...
                    let mem2 =
                        extract_bytes(current_tentative_state_bytes, self.overall_metadata.main_table_addr as nat,
                                      self.overall_metadata.main_table_size as nat);
                    let table1 =
                        parse_main_table::<K>(mem1, num_keys, main_table_entry_size).unwrap().contents;
                    let table2 =
                        parse_main_table::<K>(mem2, num_keys, main_table_entry_size).unwrap().contents;
                ...
                }),
        {
            ...
            assert(metadata_bytes =~= entry.entry->Created_0.spec_to_bytes());
            ...
            lemma_main_table_recovery_after_updating_entry::<K>(
                ...
                main_table_entry_size,
                main_table_index,
                self.main_table.outstanding_entry_writes[main_table_index].unwrap().entry->Created_0,
                key
            );
            
            assert(0 <= e.entry->Created_0.item_index < overall_metadata.num_keys);
        }
        ...
        proof fn lemma_justify_validify_log_entry(
            ...
            ensures
                ({
                    ...
                    &&& metadata == entry.entry->Created_0
                    &&& 0 <= metadata.item_index < overall_metadata.num_keys
                    &&& !main_table_view.unwrap().valid_durable_item_indices().contains(item_index)
                    &&& forall|i| 0 <= i < overall_metadata.num_keys &&
                           #[trigger] main_table_view.unwrap().contents[i] is Some
                           ==> main_table_view.unwrap().contents[i].unwrap().key != entry.key
                }),
        {
            ...
            assert forall|addr: u64| {
            ...
            } implies self_before_main_table_create.wrpm@.flush().committed()[addr as int] ==
                      self.wrpm@.flush().committed()[addr as int] by {
                ...
                if relative_addr >= index_to_offset(num_keys as nat, main_table_entry_size as nat) {
                    ...
                    assert(!old_self.get_writable_mask_for_main_table()(addr as int));
                    assert(!main_table_subregion.is_writable_absolute_addr_fn()(addr as int));
                    assert(mem1[addr as int] == mem2[addr as int]);
                }
                else {
                    ...
                    broadcast use pmcopy_axioms;
                    assert(self_before_main_table_create.wrpm@.state[addr as int].state_at_last_flush ==
                           get_subregion_view(self_before_main_table_create.wrpm@, main_table_addr as nat,
                                              main_table_size as nat).committed()[addr - main_table_addr]);
                    assert(self.wrpm@.state[addr as int].state_at_last_flush ==
                           get_subregion_view(self.wrpm@, main_table_addr as nat,
                                              main_table_size as nat).committed()[addr - main_table_addr]);
                    assert(self_before_main_table_create.wrpm@.state[addr as int] == self.wrpm@.state[addr as int]);
                }
            }
            ...
            lemma_main_table_recovery_after_updating_entry::<K>(
                ...
                main_table_entry_size,
                main_table_index,
                entry.entry->Created_0,
                key
            );

            assert(!current_tentative_main_table_parsed.valid_durable_item_indices().contains(item_index));
            assert forall|i| 0 <= i < num_keys &&
                       #[trigger] current_tentative_main_table_parsed.contents[i] is Some
                       implies current_tentative_main_table_parsed.contents[i].unwrap().key != key by {
                let table1 = mid_tentative_main_table_parsed.contents;
                let table2 = current_tentative_main_table_parsed.contents;
                assert(i != main_table_index);
                assert(table2[i] == table1[i]);
                assert(self_before_main_table_create.tentative_main_table().contents[i].unwrap().key != key);
            }
        }
                ...
                })
        {
            ...
            let item_index = match self.item_table.tentatively_write_item(
                ...
                Ghost(self.overall_metadata),
                Ghost(self.main_table@.valid_durable_item_indices()),
                Ghost(tentative_main_table_view.valid_durable_item_indices()),
            ) {
        ...
        proof fn lemma_update_item_index_log_entry_precondition(
            ...
            requires
                ...
                !old_self.main_table@.valid_durable_item_indices().contains(item_index),
                !self.main_table@.valid_durable_item_indices().contains(item_index),
                ({
                    ...
                    old_self.item_table.allocator_view().pending_alloc_check(
                        item_index,
                        old_self.main_table@.valid_durable_item_indices(),
                        tentative_main_table.valid_durable_item_indices())
                }),
                ...
                ({
                    ...
                    let old_item_table = parse_item_table::<I, K>(old_item_table_subregion.committed(), 
                        self.overall_metadata.num_keys as nat, old_main_table.valid_durable_item_indices()).unwrap();
            ...
            ensures
                ...
                ({
                    let tentative_main_table = parse_main_table::<K>(extract_bytes(tentative_view_bytes,
                        self.overall_metadata.main_table_addr as nat, self.overall_metadata.main_table_size as nat),
                        self.overall_metadata.num_keys, self.overall_metadata.main_table_entry_size).unwrap();
                    &&& !tentative_main_table.valid_durable_item_indices().contains(item_index)            
                    // &&& tentative_main_table.contents[index as int] is Some
                }),

        {
            ...
            assert(self.main_table@.valid_durable_item_indices() == old_main_table.valid_durable_item_indices());
            ...
            let tentative_item_table = parse_item_table::<I, K>(extract_bytes(tentative_view_bytes,
                self.overall_metadata.item_table_addr as nat, self.overall_metadata.item_table_size as nat),
                self.overall_metadata.num_keys as nat, tentative_main_table.valid_durable_item_indices()).unwrap();
            ...
            assert(old_self.item_table.allocator_view().pending_alloc_check(item_index,
                                                           old_main_table.valid_durable_item_indices(),
                                                           tentative_main_table.valid_durable_item_indices()));
        }
        ...
        proof fn lemma_tentative_view_after_appending_update_item_log_entry_includes_new_log_entry(
            ...
            requires 
                ...
                ({
                    ...
                    &&& old_main_table_view.contents[index as int] matches Some(entry) 
                    &&& !old_main_table_view.valid_durable_item_indices().contains(item_index)
                }),
                ({
                    ...
                    let new_main_table_view = parse_main_table::<K>(new_main_table_region,
                        self.overall_metadata.num_keys, self.overall_metadata.main_table_entry_size);
                    let old_item_index = current_durable_main_table_view.contents[index as int].unwrap().item_index();
                    ...
                    &&& new_main_table_view.unwrap().valid_durable_item_indices() == 
                            current_durable_main_table_view.valid_durable_item_indices().insert(item_index).remove(old_item_index)
                }),
                ...
                self.tentative_view().unwrap() == old_self.tentative_view().unwrap().update_item(index as int, item).unwrap(),
        {
            ...
            let old_item_index = old_tentative_main_table.contents[index as int].unwrap().item_index();
            ...
            assert forall |idx: u64| new_mem_main_table.valid_durable_item_indices().contains(idx) implies {
                ...
                if !old_tentative_main_table.valid_durable_item_indices().contains(idx) {
        ...
        proof fn lemma_pending_main_table_allocations_are_invalid(
            ...
            ensures 
                forall |idx: u64| #[trigger] old_self.main_table.allocator_view().pending_allocations.contains(idx) ==> 
                    old_self.main_table@.contents[idx as int] is None,
        {
        ...
        exec fn tentatively_write_item_helper(
            ...
            requires 
                ...
                old(self).item_table.inv(item_table_subregion.view(&old(self).wrpm), old(self).overall_metadata,
                    old(self).main_table@.valid_durable_item_indices()),
                old(self).inv(),
                ...
                match result {
                    Ok(index) => {
                        ...
                        &&& ({
                                ...
                                &&& forall |i: int| 0 <= i < self.overall_metadata.num_keys && i != index ==>
                                        #[trigger] self.item_table.outstanding_item_table@[i] == old(self).item_table.outstanding_item_table@[i]
                                &&& self.item_table.outstanding_item_table@[index as int] == Some(*item)
        ...
        exec fn create_update_item_log_entry_helper(
            ...
            requires 
                ...
                old(self).item_table.outstanding_item_table@[item_index as int] is Some,
                ({
                    ...
                    &&& tentative_main_table_view.contents[index as int] is Some
                ...
                match result {
                    Ok(log_entry) => {
                        ...
                        let current_main_table_view = parse_main_table::<K>(current_main_table_region,
                            self.overall_metadata.num_keys, self.overall_metadata.main_table_entry_size).unwrap();
                        let old_item_index = current_main_table_view.contents[index as int].unwrap().item_index();
                ...
                }
        {
            ...
            proof {
                ...
                assert(forall |i: int| 0 <= i < new_main_table_view.contents.len() && i != index ==> {
                    &&& new_main_table_view.contents[i] is None ==> current_main_table_view.contents[i] is None
                    &&& new_main_table_view.contents[i] matches Some(new_entry) ==> {
                        &&& current_main_table_view.contents[i] matches Some(current_entry)
                ...
                assert(validate_item_table_entries::<I, K>(new_item_table_region, self.overall_metadata.num_keys as nat, new_main_table_view.valid_durable_item_indices()));
                ...
                let new_list_view = DurableList::<K, L>::parse_all_lists(
                    new_main_table_view,
                    new_list_area_region,
                    self.overall_metadata.list_node_size,
                    self.overall_metadata.num_list_entries_per_node
                ...
                // TODO WITH LIST IMPLEMENTATION
                assume(new_list_view is Some);
            }
            Ok(log_entry)
        }

        // proof fn lemma_update_main_table_item_implies_overall_update(
        //     s1: Seq<u8>,
        //     s2: Seq<u8>,
        //     key: K,
        ...
        // )
        //     requires 
        //         Self::physical_recover_after_applying_log(s1, overall_metadata) is Some,
        //         Self::physical_recover_after_applying_log(s2, overall_metadata) is Some,
        //         ({
        //             let main_table_region1 = extract_bytes(s1, 
        //                 self.overall_metadata.main_table_addr as nat, self.overall_metadata.main_table_size as nat);
        //             let main_table_view1 = parse_main_table::<K>(main_table_region1,
        //                 self.overall_metadata.num_keys, self.overall_metadata.main_table_entry_size);
        //             let main_table_region2 = extract_bytes(s2, 
        //                 self.overall_metadata.main_table_addr as nat, self.overall_metadata.main_table_size as nat);
        //             let main_table_view2 = parse_main_table::<K>(main_table_region2,
        //                 self.overall_metadata.num_keys, self.overall_metadata.main_table_entry_size);
        //             &&& main_table_view1 matches Some(main_table_view1)
        //             &&& main_table_view2 matches Some(main_table_view2)
        //             &&& main_table_view1 == main_table_view.
        //         })

        //     ensures 
        //         ({
        //             let v1 = Self::physical_recover_after_applying_log(s1, overall_metadata).unwrap();
        //             let v2 = Self::physical_recover_after_applying_log(s2, overall_metadata).unwrap();
        //         })
        // {
                ...
                }
        {
            ...
            proof {
                // Prove that if we append the new log entry to the log and then install the updated log,
                // we end up in the expected state
                assert(self.wrpm@.can_crash_as(self.wrpm@.committed()));
            ...
            proof {
                ...
                // if views differ only in log region, applying the same log should result in the same state
                Self::lemma_applying_same_log_preserves_states_differ_only_in_log_region(pre_append_self.wrpm@.flush().committed(),
                    self.wrpm@.flush().committed(), self.log@.physical_op_list, self.version_metadata, self.overall_metadata);
                lemma_apply_phys_log_entries_succeeds_if_log_ops_are_well_formed(pre_append_self.wrpm@.flush().committed(),
                    self.version_metadata, self.overall_metadata, self.log@.physical_op_list);
                lemma_apply_phys_log_entries_succeeds_if_log_ops_are_well_formed(self.wrpm@.flush().committed(),
                    self.version_metadata, self.overall_metadata, self.log@.physical_op_list);
                lemma_log_replay_preserves_size(pre_append_self.wrpm@.flush().committed(), self.log@.physical_op_list);
                lemma_log_replay_preserves_size(self.wrpm@.flush().committed(), self.log@.physical_op_list);

                let current_tentative_bytes = apply_physical_log_entries(self.wrpm@.flush().committed(),
                    self.log@.physical_op_list).unwrap();
                let pre_append_tentative_bytes_with_new_log = apply_physical_log_entries(pre_append_self.wrpm@.flush().committed(),
                    self.log@.physical_op_list).unwrap();
                
                Self::lemma_if_states_differ_only_in_log_region_after_log_replay_then_views_match(
                    current_tentative_bytes, pre_append_tentative_bytes_with_new_log, self.overall_metadata);

                assert(self.inv());
                assert(self.pending_alloc_inv()) by {
                    // TODO: old self or pre append self?
                    let durable_state_bytes = self.wrpm@.committed();
                    let old_durable_state_bytes = pre_append_self.wrpm@.committed();
                    let tentative_state_bytes = apply_physical_log_entries(self.wrpm@.flush().committed(),
                        self.log@.commit_op_log().physical_op_list).unwrap();
                    // let old_tentative_state_bytes = apply_physical_log_entries(old(self).wrpm@.flush().committed(),
                    //     old(self).log@.commit_op_log().physical_op_list).unwrap();
                    let old_tentative_state_bytes = apply_physical_log_entries(pre_append_self.wrpm@.flush().committed(),
                        old(self).log@.commit_op_log().physical_op_list).unwrap();

                    let durable_main_table_region = extract_bytes(durable_state_bytes, self.overall_metadata.main_table_addr as nat, self.overall_metadata.main_table_size as nat);
                    let old_durable_main_table_region = extract_bytes(old_durable_state_bytes, self.overall_metadata.main_table_addr as nat, self.overall_metadata.main_table_size as nat);

                    let tentative_main_table_region = extract_bytes(tentative_state_bytes, self.overall_metadata.main_table_addr as nat, self.overall_metadata.main_table_size as nat);
                    let old_tentative_main_table_region = extract_bytes(old_tentative_state_bytes, self.overall_metadata.main_table_addr as nat, self.overall_metadata.main_table_size as nat);


                    assert(forall |s| #[trigger] self.wrpm@.can_crash_as(s) ==> 
                        Self::physical_recover(s, self.version_metadata, self.overall_metadata) == Some(self@));
                    assert(get_subregion_view(self.wrpm@, self.overall_metadata.main_table_addr as nat,
                        self.overall_metadata.main_table_size as nat).committed() == durable_main_table_region);
                    assert(get_subregion_view(self.wrpm@, self.overall_metadata.main_table_addr as nat,
                        self.overall_metadata.main_table_size as nat).can_crash_as(durable_main_table_region));

                    assert(forall |s| #[trigger] pre_append_self.wrpm@.can_crash_as(s) ==> 
                        Self::physical_recover(s, self.version_metadata, self.overall_metadata) == Some(old(self)@));
                    assert(get_subregion_view(pre_append_self.wrpm@, self.overall_metadata.main_table_addr as nat,
                        self.overall_metadata.main_table_size as nat).committed() == old_durable_main_table_region);
                    assert(get_subregion_view(pre_append_self.wrpm@, self.overall_metadata.main_table_addr as nat,
                        self.overall_metadata.main_table_size as nat).can_crash_as(old_durable_main_table_region));

                    assert(old_durable_main_table_region == durable_main_table_region);

                    let durable_main_table_view = parse_main_table::<K>(durable_main_table_region, self.overall_metadata.num_keys,
                        self.overall_metadata.main_table_entry_size).unwrap();
                    let old_durable_main_table_view = parse_main_table::<K>(old_durable_main_table_region, self.overall_metadata.num_keys,
                        self.overall_metadata.main_table_entry_size).unwrap();    
                    let tentative_main_table_view = parse_main_table::<K>(tentative_main_table_region, self.overall_metadata.num_keys,
                        self.overall_metadata.main_table_entry_size).unwrap();
                    let old_tentative_main_table_view = parse_main_table::<K>(old_tentative_main_table_region, self.overall_metadata.num_keys,
                        self.overall_metadata.main_table_entry_size).unwrap();

                    assert(self.main_table@.valid_durable_item_indices() == durable_main_table_view.valid_durable_item_indices());
                    assert(old_durable_main_table_view.contents.len() == durable_main_table_view.contents.len());
                    assert(old_tentative_main_table_view.contents.len() == tentative_main_table_view.contents.len());
                    assert(old(self).pending_alloc_inv());

                    assert(old_tentative_main_table_view.update_item_index(offset as int, item_index).unwrap() == tentative_main_table_view);
                    
                    assert forall |idx: u64| 0 <= idx < durable_main_table_view.contents.len() implies 
                        self.main_table.allocator_view().pending_alloc_check(idx, durable_main_table_view, tentative_main_table_view) 
                    by {
                        // assert(old(self).main_table.allocator_view().pending_alloc_check(idx, old_durable_main_table_view, old_tentative_main_table_view));
                        // assert(durable_main_table_view.contents[idx as int] == old_durable_main_table_view.contents[idx as int]);
                        if idx != offset {
                            assert(old(self).main_table.allocator_view().pending_alloc_check(idx, old_durable_main_table_view, old_tentative_main_table_view));
                            // assert(tentative_main_table_view.contents[idx as int] == old_tentative_main_table_view.contents[idx as int]);
                        } else {
                            assert(idx == offset);
                            // PROBLEM: pending alloc inv says that the entries match if they 
                            // are both Some, but that's not true here -- they have different 
                            // item indices
                            // assume(false);
                    ...
                    // assert(self.main_table.pending_alloc_inv(
                    //     durable_main_table_region,
                    //     tentative_main_table_region,
                    //     self.overall_metadata
                    // ));
        ...
        proof fn lemma_tentative_view_after_appending_delete_log_entry_includes_new_log_entry(
            ...
            requires 
                old_self.valid(),
                old_self.main_table.tentative_view().contents[index as int] is Some,
                !old_self.transaction_committed(),
                !self.transaction_committed(),
                old_self.pending_alloc_inv(),
                forall |s| #[trigger] old_self.wrpm@.can_crash_as(s) ==> 
                ...
                0 <= index < self.main_table@.contents.len(),
                ({
                    let old_flushed_mem = old_self.wrpm@.flush().committed();
                    let old_op_log = old_self.log@.commit_op_log().physical_op_list;
                    let old_mem_with_old_log_installed = apply_physical_log_entries(old_flushed_mem, old_op_log);
                ...
                ({
                    ...
                    let new_main_table_view = parse_main_table::<K>(new_main_table_region,
                        self.overall_metadata.num_keys, self.overall_metadata.main_table_entry_size);
                    // let item_index = self.main_table@.contents[index as int].unwrap().item_index();
                    let item_index = if self.main_table.outstanding_entry_writes[index] is Some {
                        self.main_table.outstanding_entry_writes[index].unwrap()@.unwrap().entry.item_index
                    } else {
                        self.main_table@.contents[index as int].unwrap().entry.item_index
                    };
                    &&& new_main_table_view is Some
                    &&& new_main_table_view == current_main_table_view.delete(index as int)
                    &&& new_main_table_view.unwrap().valid_durable_item_indices() == current_main_table_view.valid_durable_item_indices().remove(item_index)
                }),
            ...
            ensures 
                ({
                    ...
                    &&& main_table_view.valid_durable_item_indices() == old_main_table_view.valid_durable_item_indices().remove(item_index)
                ...
                })
        {
            ...
            assert(old_main_table_view.contents[index as int] matches Some(entry) && entry.item_index() == item_index);
            assert(main_table_view.valid_durable_item_indices() == old_main_table_view.valid_durable_item_indices().remove(item_index));
        ...
        proof fn lemma_item_index_is_currently_valid(self, index: u64, item_index: u64)
            requires 
                ...
                // self.main_table@.valid_durable_item_indices().contains(item_index),
                ({
                    &&& self.main_table@.contents[index as int] matches Some(entry)
                    &&& entry.item_index() == item_index
                }),
            ensures 
                !self.item_table.pending_allocations_view().contains(item_index),
        ...
        pub fn tentative_delete(
            ...
            requires
                old(self).valid(),
                old(self).outstanding_view().contains_key(index as int),
                !old(self).transaction_committed(),
                !old(self).pending_deallocations().contains(index),
                old(self).pending_alloc_inv(),
                old(self).pending_alloc_inv2(),
                ...
                }
        {
            ...
            proof {
                ...
                assert(main_table_subregion_view.flush() == get_subregion_view(self.wrpm@.flush(),
                    self.overall_metadata.main_table_addr as nat, self.overall_metadata.main_table_size as nat));
                assert(parse_main_table::<K>(extract_bytes(old_tentative_view_bytes, self.overall_metadata.main_table_addr as nat,
                    self.overall_metadata.main_table_size as nat), self.overall_metadata.num_keys, self.overall_metadata.main_table_entry_size) is Some);
            }
            ...
            let ghost new_tentative_view_bytes = apply_physical_log_entries(self.wrpm@.flush().committed(),
                self.log@.commit_op_log().physical_op_list).unwrap();
            ...
            proof {
                ...
                assert forall |idx: u64| 0 <= idx < durable_main_table_view.contents.len() && idx != index implies 
                    self.main_table.allocator_view().pending_alloc_check(idx, durable_main_table_view, tentative_main_table_view)
            ...
            proof {
                ...
                assert(self.main_table@.valid_durable_item_indices() == old(self).main_table@.valid_durable_item_indices());

                assert(tentative_main_table_view.valid_durable_item_indices() == old_tentative_main_table_view.valid_durable_item_indices().remove(item_index));
                ...
                by { 
                    assert(old(self).item_table.allocator_view().pending_alloc_check(idx,
                                                                  self.main_table@.valid_durable_item_indices(),
                                                                  old_tentative_main_table_view.valid_durable_item_indices()));
                ...
                assert(!tentative_main_table_view.valid_durable_item_indices().contains(item_index));
            ...
            self.item_table.tentatively_deallocate_item(Ghost(item_table_subregion_view), item_index, 
                Ghost(self.overall_metadata), Ghost(self.main_table@.valid_durable_item_indices()),
                Ghost(tentative_main_table_view.valid_durable_item_indices()), Ghost(new_tentative_view_bytes));
                ...
                }
        {
            ...
            assert(self.item_table.opaquable_inv(self.overall_metadata, self.main_table@.valid_durable_item_indices()));
            ...
            assert(self.item_table.opaquable_inv(self.overall_metadata, old(self).main_table@.valid_durable_item_indices()));

            self.item_table.finalize_item_table(Ghost(old(self).item_table), Ghost(self.wrpm@), 
                Ghost(self.overall_metadata), Ghost(old(self).main_table@.valid_durable_item_indices()), Ghost(self.main_table@.valid_durable_item_indices()));

...
//! regions for use in a singlelog.
//!
...
verus! {
    ...
    // space on persistent memory regions to support a singlelog.
    //
    ...
    //
    // The return value is a `Result<(), LogErr>`, meaning the following:
    //
    // `Ok(())` -- there's enough space on each region
    // `Err(err)` -- there isn't enough space, so the caller should return the error `err`.
    pub fn check_for_required_space(region_sizes: &Vec<u64>, num_regions: u32) -> (result: Result<(), LogErr>)
        requires
    ...
    // various information about this region and the singlelog it's
    // part of, the function needs some parameters:
    //
    // `region_size`: how big this region is
    // `log_id`: the GUID of the singlelog it's being used for
            ...
            })
    {
        ...
        // Append the little-endian encoding of the singlelog ID.
        let mut t = u128_to_le_bytes(log_id);
        result.append(&mut t);
    ...
    // metadata about this region and the singlelog it's part of, the
    ...
    // singlelog. To do so, it needs various metadata about this region
    // and the singlelog it's part of, so it needs some parameters:
    //
    // `region_size`: how big this region is
    // `log_id`: the GUID of the singlelog it's being used for
    // `num_logs`: the number of logs in the singlelog
            ...
            pm_regions@.current_timestamp.device_id() == old(pm_regions)@.current_timestamp.device_id()
    {
        ...
        // Also, compute a CRC of that encoding and append that.
        let mut t = compute_level2_metadata_encoded(region_size, log_id, num_logs, which_log);
        let mut c = bytes_crc(t.as_slice());
    ...
    // all the metadata necessary to set up a singlelog. To do so, it
    ...
    //
    // ```
    // recover_all(pm_regions@.committed(), log_id) ==
    //     Some(AbstractLogState::initialize(log_capacities))
    // ```
    //
    ...
    pub fn write_setup_metadata_to_all_regions<PMRegions: PersistentMemoryRegions>(
        ...
        ensures
            ...
            recover_all(pm_regions@.committed(), log_id) == Some(AbstractLogState::initialize(log_capacities)),
            ...
            pm_regions@.current_timestamp.device_id() == old(pm_regions)@.current_timestamp.device_id()
    {
            ...
            invariant
                ...
                forall |i: u32| i < which_log ==>
                    memory_correctly_set_up_on_single_region(#[trigger] pm_regions@[i as int].flush().committed(),
                                                             region_sizes@[i as int], log_id, num_logs, i),
                ...
                pm_regions@.current_timestamp.device_id() == old_pm_regions.current_timestamp.device_id()
        {
            ...
            write_setup_metadata_to_single_region(pm_regions, region_size, log_id, num_logs, which_log);
        }
        ...
        proof {
            ...
            assert(recover_all(pm_regions_committed, log_id)
                   =~= Some(AbstractLogState::initialize(log_capacities))) by {

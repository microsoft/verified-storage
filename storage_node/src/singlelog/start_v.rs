...
//! as a singlelog. Such starting is done either after setup or after a
//! crash.
//!
...
use crate::singlelog::singlelogimpl_t::LogErr;
...
verus! {
    ...
    pub fn read_cdb<PMRegions: PersistentMemoryRegions>(pm_regions: &PMRegions) -> (result: Result<bool, LogErr>)
        requires
            pm_regions.inv(),
            pm_regions@.len() > 0,
    ...
    //
    // `which_log` -- which among the singlelog's logs to read
    //
    ...
    //
    // `Err(LogErr::StartFailedDueToMultilogIDMismatch)` -- The
    // singlelog ID stored in persistent memory doesn't match the one
    ...
    fn read_log_variables<PMRegions: PersistentMemoryRegions>(
        ...
        ensures
            ({
                ...
                let state = recover_abstract_log_from_region_given_cdb(pm_regions@[w].committed(), log_id,
                                                                       num_logs as int, w, cdb);
                match result {
                    Ok(info) => state.is_Some() ==> {
                        &&& metadata_consistent_with_info(pm_regions@[w], log_id, num_logs, which_log, cdb, info)
            ...
            })
    {
        ...
        let ghost state = recover_abstract_log_from_region_given_cdb(pm_regions@[w].committed(), log_id,
                                                                     num_logs as int, w, cdb);
        ...
        let log_id_read = u128_from_le_bytes(slice_subrange(
            level2_metadata_bytes.as_slice(),
            RELATIVE_POS_OF_LEVEL2_MULTILOG_ID as usize,
            RELATIVE_POS_OF_LEVEL2_MULTILOG_ID as usize + 16));
        if log_id_read != log_id {
            assert(state.is_None()); // This can't happen if the persistent memory is recoverable
            return Err(LogErr::StartFailedDueToMultilogIDMismatch{
                which_log,
    ...
    ) -> (result: Result<Vec<LogInfo>, LogErr>)
        requires
            pm_regions.inv(),
            ...
            recover_given_cdb(pm_regions@.committed(), log_id, cdb) == Some(state),
        ensures
            match result {
                Ok(info) => {
                    &&& each_metadata_consistent_with_info(pm_regions@, log_id, num_regions, cdb, info@)
            ...
            }
    {
        ...
        for which_log in 0..num_regions
            invariant
                ...
                forall |j:u32| j < which_log ==> {
                    ...
                    &&& metadata_consistent_with_info(pm_regions@[j as int], log_id, num_regions, j, cdb,
                                                    infos[j as int])
                },
                num_regions == pm_regions@.len(),
                recover_given_cdb(pm_regions@.committed(), log_id, cdb) == Some(state),
                pm_regions@.no_outstanding_writes(),
        {
            ...
            let ghost region_state = recover_abstract_log_from_region_given_cdb(
                pm_regions@[which_log as int].committed(), log_id, num_regions as int, which_log as int, cdb);
            assert (region_state.is_Some()) by {
                ...
                let ghost seq_option = pm_regions@.committed().map(
                    |idx, c| recover_abstract_log_from_region_given_cdb(c, log_id, num_regions as int,
                                                                        idx, cdb));
                // Then, we trigger it by mentioning `seq_option[which_log as int]`.
                assert(region_state == seq_option[which_log as int]);
            }

            let info = read_log_variables(pm_regions, log_id, cdb, num_regions, which_log)?;
            infos.push(info);
        }
        Ok(infos)
    }

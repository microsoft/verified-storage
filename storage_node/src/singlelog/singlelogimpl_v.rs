...
//! which implements a provably correct singlelog.
//!
...
use crate::singlelog::append_v::*;
use crate::singlelog::inv_v::*;
use crate::singlelog::layout_v::*;
use crate::singlelog::setup_v::{
...
use crate::singlelog::start_v::{read_cdb, read_logs_variables};
use builtin::*;
use builtin_macros::*;
...
verus! {
    ...
    // `state` -- the abstract view of the singlelog
    pub struct UntrustedLogImpl<
        ...
        H: Sized + Serializable,
        D: Sized + Serializable,
        // H: Serializable,
        // D: Serializable
    > {
        num_logs: u32,
        ...
        _phantom: Ghost<Option<(H, D)>> // TODO: use PhantomData
    }
    ...
    impl<
        ...
        // H: Serializable,
        // D: Serializable
    > UntrustedLogImpl<CDBRegion, HRegion, DRegion, H, D>
    {
        // This static function specifies how multiple regions'
        // contents should be viewed upon recovery as an abstract
        // singlelog state.
        pub closed spec fn recover(mems: Seq<Seq<u8>>, log_id: u128) -> Option<AbstractLogState>
        {
            recover_all(mems, log_id)
        }
                ...
                PMRegions: PersistentMemoryRegions
        {
            ...
            &&& each_metadata_consistent_with_info(wrpm_regions@, log_id, self.num_logs, self.cdb, self.infos@)
            &&& each_info_consistent_with_log_area(wrpm_regions@, self.num_logs, self.infos@, self.state@)
            &&& can_only_crash_as_state(wrpm_regions@, log_id, self.state@.drop_pending_appends())
        }
        ...
        pub proof fn lemma_inv_implies_can_only_crash_as<Perm, PMRegions>(
            ...
            requires
                self.inv(wrpm_regions, log_id)
            ensures
            can_only_crash_as_state(wrpm_regions@, log_id, self@.drop_pending_appends())
        {}

        // This function specifies how to view the in-memory state of
        // `self` as an abstract singlelog state.
            ...
            ensures
                ...
                match result {
                    Ok(log_capacities) => {
                        ...
                        &&& Self::recover(pm_regions@.committed(), log_id) == Some(state)
                        &&& Self::recover(pm_regions@.flush().committed(), log_id) == Some(state)
                ...
                }
        {
            ...
            // storing a singlelog. If not, return an appropriate
            // error.
            ...
            if num_regions < 1 {
                return Err(LogErr::CantSetupWithFewerThanOneRegion { });
            }
            if num_regions > u32::MAX as usize {
                return Err(LogErr::CantSetupWithMoreThanU32MaxRegions { });
            }
            ...
            write_setup_metadata_to_all_regions(pm_regions, &region_sizes, Ghost(log_capacities@), log_id);
        ...
        pub exec fn start<PMRegions>(
            ...
            requires
                ({
                    ...
                    Self::recover(flushed.committed(), log_id) == Some(state)
                }),
            ...
            ensures
                ...
                match result {
                    Ok(log_impl) => {
                        &&& log_impl.inv(wrpm_regions, log_id)
                        &&& log_impl@ == state
                        &&& can_only_crash_as_state(wrpm_regions@, log_id, state.drop_pending_appends())
                    ...
                    Err(LogErr::CRCMismatch) => !wrpm_regions.constants().impervious_to_corruption,
                    _ => false
                }
        {
            ...
            let infos = read_logs_variables(pm_regions, log_id, cdb, num_logs, Ghost(state))?;
            proof {
                ...
                lemma_invariants_imply_crash_recover_forall(pm_regions@, log_id, num_logs, cdb,
                                                            infos@, state);
                lemma_recovered_state_is_crash_idempotent(wrpm_regions@.committed(), log_id);
            }
        ...
        // the singlelog. It's tentative in that crashes will undo the
        ...
        pub exec fn tentatively_append<PMRegions>(
            ...
            where
                PMRegions: PersistentMemoryRegions
            requires
                old(self).inv(&*old(wrpm_regions), log_id),
                ...
                }
        {
            ...
            if which_log >= self.num_logs {
                return Err(LogErr::InvalidLogIndex{ });
            }
            ...
            if info.log_plus_pending_length >= info.log_area_len - info.head_log_area_offset {
                ...
                proof {
                    lemma_tentatively_append(wrpm_regions@, log_id, self.num_logs, which_log,
                ...
                if num_bytes <= max_len_without_wrapping {
                    ...
                    proof {
                        lemma_tentatively_append_wrapping(wrpm_regions@, log_id, self.num_logs, which_log,
                                                          bytes_to_append@, self.cdb, self.infos@, self.state@);
                    }
        ...
        exec fn update_level3_metadata<PMRegions>(
            ...
            requires
                ...
                each_metadata_consistent_with_info(old(wrpm_regions)@, log_id, old(self).num_logs,
                ...
                wrpm_regions@.current_timestamp.device_id() == old(wrpm_regions)@.current_timestamp.device_id(),
        {
            ...
            for current_log in 0..self.num_logs
                invariant
                    ...
                    memory_matches_cdb(wrpm_regions@, self.cdb),
                    each_metadata_consistent_with_info(wrpm_regions@, log_id, self.num_logs, self.cdb, prev_infos),
                    ...
                    forall |s| Self::recover(s, log_id) == Some(prev_state.drop_pending_appends()) ==>
                        #[trigger] perm.check_permission(s),
                    ...
                    forall |which_log: u32| #[trigger] is_valid_log_index(which_log, self.num_logs)
                        ...
                        let w = which_log as int;
                        let flushed = wrpm_regions@.flush();
                        metadata_consistent_with_info(flushed[w], log_id, self.num_logs, which_log,
                    ...
                    wrpm_regions@.current_timestamp == old_timestamp,
            {
                ...
                proof {
                    lemma_updating_inactive_metadata_maintains_invariants(
                        wrpm_regions@, log_id, self.num_logs, self.cdb, prev_infos, prev_state, current_log,
                        bytes_to_write@);
                }
                ...
                assert forall |crash_bytes| wrpm_regions_new.can_crash_as(crash_bytes)
                    ...
                    lemma_invariants_imply_crash_recover_forall(
                        wrpm_regions_new, log_id, self.num_logs, self.cdb, prev_infos, prev_state);
                }
                ...
                assert (metadata_consistent_with_info(flushed[current_log as int], log_id,
            ...
            proof {
                lemma_flushing_metadata_maintains_invariants(wrpm_regions@, log_id, self.num_logs, self.cdb,
            ...
            assert forall |which_log: u32| #[trigger] is_valid_log_index(which_log, self.num_logs) implies {
                let w = which_log as int;
                &&& metadata_consistent_with_info(pm_regions_after_flush[w], log_id, self.num_logs, which_log,
                                                 !self.cdb, self.infos@[w])
            ...
            proof {
                lemma_invariants_imply_crash_recover_forall(pm_regions_after_flush, log_id,
            ...
            assert forall |crash_bytes| pm_regions_after_write.can_crash_as(crash_bytes) implies
                ...
                lemma_invariants_imply_crash_recover_forall(wrpm_regions@, log_id, self.num_logs,
                ...
                }
        {
            ...
            self.update_level3_metadata(wrpm_regions, Ghost(log_id), Ghost(prev_infos),
                ...
                }
        {
            ...
            proof {
                assert(is_valid_log_index(0, self.num_logs));
                lemma_invariants_imply_crash_recover_forall(wrpm_regions@, log_id, self.num_logs, self.cdb,
            ...
            if new_head < info.head {
                return Err(LogErr::CantAdvanceHeadPositionBeforeHead{ head: info.head })
            }
            if new_head - info.head > info.log_length as u128 {
                return Err(LogErr::CantAdvanceHeadPositionBeyondTail{ tail: info.head + info.log_length as u128 })
            }
            ...
            self.update_level3_metadata(wrpm_regions, Ghost(log_id), Ghost(prev_infos), Ghost(prev_state),
                                        Tracked(perm));
        ...
        proof fn lemma_read_of_continuous_range(
            ...
            requires
                ...
                len > 0,
                each_metadata_consistent_with_info(pm_regions_view, log_id, self.num_logs, self.cdb, self.infos@),
                ...
                })
        {
            ...
            if relative_pos >= log_area_len - info.head_log_area_offset {
                ...
                proof { self.lemma_read_of_continuous_range(pm_regions@, log_id, which_log, pos as int,
            ...
            proof {
                self.lemma_read_of_continuous_range(pm_regions@, log_id, which_log, pos as int,
            ...
            proof {
                self.lemma_read_of_continuous_range(pm_regions@, log_id, which_log,
        ...
        // We have to go through UntrustedLogImpl to update timestamps, even though
        ...
        pub fn update_timestamps<Perm, PMRegions>(
            ...
            where
                ...
                PMRegions: PersistentMemoryRegions
            requires
                self.inv(&*old(wrpm_regions), log_id),

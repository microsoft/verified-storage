...
verus! {
            ...
            ({
                ...
                let write_addr = ABSOLUTE_POS_OF_LOG_AREA +
                    relative_log_pos_to_log_area_offset(prev_info.log_plus_pending_length as int,
                                                        ...
                                                        log_area_len as int);
                let (pm_regions_view2, new_timestamp) = pm_regions_view.write(which_log as int, write_addr, bytes_to_append, timestamp);
            ...
            ({
                ...
                let write_addr = ABSOLUTE_POS_OF_LOG_AREA +
                    relative_log_pos_to_log_area_offset(prev_info.log_plus_pending_length as int,
                                                        prev_info.head_log_area_offset as int,
                                                        log_area_len as int);
                let (pm_regions_view2, timestamp2) = pm_regions_view.write(w, write_addr, bytes_to_append_part1, timestamp);
                let (pm_regions_view3, timestamp3) = pm_regions_view2.write(w, ABSOLUTE_POS_OF_LOG_AREA as int, bytes_to_append_part2, timestamp2);
                &&& each_metadata_consistent_with_info(pm_regions_view, multilog_id, num_logs, cdb, new_infos)
            ...
            }),
    {
        ...
        lemma_tentatively_append(pm_regions_view2, multilog_id, num_logs, which_log, bytes_to_append_part2, cdb,
                                 intermediate_infos, intermediate_state, timestamp2);

...
verus! {
        ...
        ensures 
            ...
            match result {
                Ok(()) => {
                    ...
                    let state = AbstractLogState::initialize(log_size - log_header_area_size());
                    let recovered_state = recover(extract_bytes(pm, log_start_addr as nat, log_size as nat), log_id).unwrap();
            ...
            
    {
        ...
        proof { 
            ...
            let recovered_state = recover(extract_bytes(pm, log_start_addr as nat, log_size as nat), log_id);

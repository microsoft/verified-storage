...
- Macrobenchmarks:
   - Figure 3: Most systems achieve higher throughput on most workloads, although note that the absolute numbers will likely vary between CPUs. Since the biggest factor in pmem-Redis' performance is client-server communication, we expect that it will see less performance improvement than the other systems. Viper may see more improvement/perform better than CapybaraKV, which we attribute largely to the increased impact of CPU performance on emulated PM compared to Optane, since CapybaraKV uses CRCs and Viper does not.
...
- Startup times: We do not expect significant differences in startup times between Optane and emulated PM for pmem-Redis and pmem-RocksDB. We see improved empty and full start times for both CapybaraKV and Viper, which are due to emulated PM's faster performance and the smaller KV store size. 

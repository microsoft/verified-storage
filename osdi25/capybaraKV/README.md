...
4. Run the full performance experiments and generate plots/tables corresponding to those in the paper. We provide scripts and and configuration files for these experiments. It will take about 12 hours total to run these experiments. 
...
##### YCSB macrobenchmarks and memory/storage utilization (~8 hours)
...
- The output of `get_mem_and_storage_use.py` and `get_startup_times.py` is similar/shows similar performance patterns to the versions of these plots in the paper.
   - We expect some variation in memory usage and startup times. We measure system-wide memory usage before and after running YCSB LoadA and report the difference, so some noise is expected. 
   - The storage usage numbers reported in the paper are rounded to the nearest whole number. We do not expect any variation in these measurements.
...
DRAM has different performance characteristics than Optane PM, including lower latency and higher bandwidth, so some nontrivial performance differences are expected. 
We ran the experiments on a server with an Intel Xeon Gold 6242 CPU, 32 physical cores, and 192GiB total memory, 64GiB of which was used to emulate PM, and observed the following differences:
- Microbenchmarks (Figure 2): all systems have lower latency; many operations are sub-1usec. The high-level performance patterns are otherwise similar to those reported in the paper.
- Macrobenchmarks:
   - Figure 3: Most systems achieve higher throughput on most workloads, although note that the absolute numbers will vary between different machines. Since the biggest factor in pmem-Redis' performance is client-server communication, we expect that it will see less performance improvement than the other systems. Viper may see more improvement/perform better than CapybaraKV, which we attribute largely to the increased impact of CPU performance on emulated PM compared to Optane, since CapybaraKV uses CRCs and Viper does not.
   - Figure 4: Many workloads may scale better than reported in the paper, particularly non-Load, non-read-heavy workloads, which do not scale well on PM. We attribute this to reduced lock contention due to lower per-operation latency.
- Memory and storage usage: We do not expect significant differences here between Optane and emulated PM.
- Startup times: We do not expect significant differences in startup times between Optane and emulated PM for pmem-Redis and pmem-RocksDB. We see improved empty and full start times for both CapybaraKV and Viper, which are due to emulated PM's faster performance and the smaller KV store size. Unlike the numbers reported in the paper on Optane PM, we found that Viper starts faster in both cases and has a much lower full start time than CapybaraKV in this setting, which we attribute mostly to the smaller number of PM files that Viper memory-maps during setup in this experiment.
